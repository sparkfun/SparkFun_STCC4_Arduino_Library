...
class sfDevSTCC4
{
  public:
     ...
     * validating its product ID. The identity check requires the sensor to be in idle state, so if
     * the sensor was left asleep or left measuring (for example, the controller was reset or
     * re-flashed mid-sketch while the sensor kept bus power), this method recovers automatically by
     * waking it or stopping the running measurement. The sensor is always idle when this returns
     * ::ksfTkErrOk. Worst case, the measurement-stop recovery blocks for its 1.2 s execution time.
     *
     ...
     *
     * @details The STCC4 soft reset is issued as an I2C general call: the single-byte reset command
     * is sent to address 0x00 rather than the sensor's own address, and is not acknowledged. This
     * method temporarily re-points the bus at the general call address to send the command, restores
     * the sensor address, and waits for the reset to complete. The sensor returns to the same state
     * as after a power cycle (idle mode, default compensation values).
     *
     * @note All devices on the bus that respond to an I2C general call reset will also reset.
     *
     * @return ::ksfTkErrOk on success, ::ksfTkErrBusNotInit if the bus is not initialized.
     */
     ...
     *
     * @param maxAttempts Total number of read attempts before giving up (default 4). A value of 0
     * is treated as 1 (at least one attempt is always made).
  ...
  protected:
    ...
    // Precomputed reciprocals / scale factors so the conversion math uses multiplication instead of
    // (much slower) floating-point division on low-end MCUs. These fold at compile time.
    static constexpr float kTicksFullScaleInv = 1.0f / kTicksFullScale; ///< 1 / full-scale tick count.
    static constexpr float kTempTicksPerDegreeC =
        kTicksFullScale / kTemperatureSlopeC; ///< Input ticks per °C (= 65535 / 175).
    static constexpr float kHumidityTicksPerPercent =
        kTicksFullScale / kHumiditySlope; ///< Input ticks per %RH (= 65535 / 125).
